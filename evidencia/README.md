...
|---|---|---|
| [`corrida_1_indexacion.txt`](corrida_1_indexacion.txt) | `python -m src.main` con `./vectorstore` borrado | Ingesta: 4 documentos → 24 fragmentos persistidos en la colección `manuales_tecnicos_dev`; retrieval con `top_k=4`; 4 respuestas con contexto y 1 "No lo sé"; cada respuesta con sus `fragmentos_recuperados` (fuente, sección y similitud) |
...
| [`tests_pytest.txt`](tests_pytest.txt) | `pytest -v` | 39 tests en verde, sin llamadas de red ni API keys |
